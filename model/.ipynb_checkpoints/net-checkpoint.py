...
from transformers import BertModel, GPT2Tokenizer, GPT2Model
from torchmetrics.classification import MulticlassF1Score, MulticlassRecall
import utils
from model.data_loader import Mention
...
class Net(nn.Module):
    ...
    def forward(self, sentence, clusters, mode='training'):
        ...
        coreference_total_scores_max_index = torch.LongTensor()
    ...
    def _extract_attention(self, mentions):
        for mention in mentions:
            if self.technique == 'head':
                ...
                weights = torch.FloatTensor([[len(mention.words)]])
                for i in range(len(mention.words)):
                    if i != mention.head:
                        encodings = torch.cat((encodings, mention.words[i].encoding), dim=0)
                        weights = torch.cat((weights, torch.FloatTensor([[1]])), dim=0)                    
